...
namespace SFA.DAS.FAA.Data.UnitTests.ElasticSearch
{
    public class WhenBuildingFindVacanciesQuery
    {
            ...
            ElasticSearchQueryBuilder queryBuilder)
        {
            ...
            //Assert
            query.Should().Be(@$"{{ ""terms"": {{ ""standardLarsCode"": [""{string.Join(@""",""", model.LarsCode)}""] }} }}");
        }
            ...
            ElasticSearchQueryBuilder queryBuilder)
        {
            ...
            //Assert
            query.Should().Be(@$"{{ ""geo_distance"": {{ ""distance"": ""{model.DistanceInMiles}miles"", ""location"": {{ ""lat"": {model.Lat}, ""lon"": {model.Lon} }} }} }}, {{ ""range"": {{ ""postedDate"": {{ ""gte"": ""now-{model.PostedInLastNumberOfDays}d/d"", ""lt"": ""now/d"" }} }} }}, {{ ""terms"": {{ ""category"": [""{string.Join(@""",""", model.Categories)}""] }} }}, {{ ""terms"": {{ ""standardLarsCode"": [""{string.Join(@""",""", model.LarsCode)}""] }} }}");
        }
